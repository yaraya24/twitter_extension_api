...
def statistics():
    ...
    tweets_by_source_result = db.session.execute(
        "SELECT COUNT(*), source FROM tweets GROUP BY source"
    )
    statistics["Tweets_by_source"] = []
    for r in tweets_by_source_result:
        statistics["Tweets_by_source"].append(dict(r))
    most_followed = db.session.execute(
        """SELECT t1.username, t2.count as number_of_followers
                                        FROM users as t1
                                        INNER JOIN (
    ...
    most_active_user = db.session.execute(
        """ SELECT t1.username, t2.count + t3.count AS total_contributions
                                            FROM users as t1
                                            INNER JOIN (
